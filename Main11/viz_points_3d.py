...
def run_viewer(sim, width=1000, height=800, print_timings=False, sim_ups=120):
    if not glfw.init():
    ...
    glfw.swap_interval(0)  # uncapped: sim steps run as fast as possible with fixed dt
    ...
    # Fixed-timestep accumulator for decoupled sim UPS
    sim_dt = 0.05
    sim_step_interval = 1.0 / sim_ups  # wall-clock seconds per sim tick
    sim_accumulator = 0.0
    max_steps_per_frame = 8  # spiral-of-death guard: drop steps if sim can't keep up
    ...
    while not glfw.window_should_close(window):
        ...
        # Advance simulation at its own fixed rate, independent of render FPS
        if not paused:
            sim_accumulator += elapsed
            steps_this_frame = 0
            while sim_accumulator >= sim_step_interval and steps_this_frame < max_steps_per_frame:
                sim.step(sim_dt, print_timings=print_timings)
                sim_accumulator -= sim_step_interval
                steps_this_frame += 1
            if steps_this_frame == max_steps_per_frame and sim_accumulator >= sim_step_interval:
                sim_accumulator = 0.0  # discard backlog to prevent spiral-of-death
        if step_once:
            sim.step(sim_dt, print_timings=print_timings)
            step_once = False
