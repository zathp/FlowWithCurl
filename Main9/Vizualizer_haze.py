# Vizualizer_haze.py
"""
Volume haze renderer for a 3D density field with:
- Debug skybox (solid colors by axis)
- Optional point-cloud overlay (one point per voxel) from sim.build_point_vertices()

Controls
- Space : pause / unpause (starts paused)
- N     : single-step one sim tick (when paused)
- R     : reset/seed density
- B     : toggle skybox
- P     : toggle points overlay
- Esc   : quit
"""
...
# Points shaders (same structure as Vizualizer.py)
POINTS_VERT = r"""
#version 330 core
...
def _update_density_3d_texture(tex, density_np):
    if density_np.dtype != np.float32:
        density_np = density_np.astype(np.float32, copy=False)
...
def _upload_density_3d(density_cp, tex):
    density_np = density_cp.astype(np.float32, copy=False).get()
    _update_density_3d_texture(tex, density_np)


def _init_density(sim):
    import cupy as cp
    d = getattr(sim, "densityfield", None)
    if d is None or not isinstance(d, cp.ndarray):
        raise RuntimeError("sim.densityfield must be a CuPy array")
...
def _setup_points(sim):
    ...
    vbo = gl.glGenBuffers(1)
    gl.glBindBuffer(gl.GL_ARRAY_BUFFER, vbo)
    gl.glBufferData(gl.GL_ARRAY_BUFFER, num_voxels * 6 * float_size, None, gl.GL_DYNAMIC_DRAW)
    ...
    gl.glBindVertexArray(0)
    return vao, vbo, num_voxels


def _upload_points(sim, vbo):
    verts_points_gpu = sim.build_point_vertices()  # (NZ, NY, NX, 6)
    verts_points_cpu = verts_points_gpu.get().astype(np.float32, copy=False).ravel()

    gl.glBindBuffer(gl.GL_ARRAY_BUFFER, vbo)
    gl.glBufferSubData(gl.GL_ARRAY_BUFFER, 0, verts_points_cpu.nbytes, verts_points_cpu)
    gl.glBindBuffer(gl.GL_ARRAY_BUFFER, 0)
...
def run_viewer(sim):
    ...
    window = glfw.create_window(1200, 800, "Haze + Points (P points, B skybox)", None, None)
    if not window:
        glfw.terminate()
    ...
    # Programs
    sky_prog = compileProgram(compileShader(SKYBOX_VERT, gl.GL_VERTEX_SHADER),
                              compileShader(SKYBOX_FRAG, gl.GL_FRAGMENT_SHADER))
    vol_prog = compileProgram(compileShader(VOLUME_VERT, gl.GL_VERTEX_SHADER),
                              compileShader(VOLUME_FRAG, gl.GL_FRAGMENT_SHADER))
    pts_prog = compileProgram(compileShader(POINTS_VERT, gl.GL_VERTEX_SHADER),
                              compileShader(POINTS_FRAG, gl.GL_FRAGMENT_SHADER))

    # VAOs
    sky_vao = _create_fullscreen_vao()
    dummy_vao = gl.glGenVertexArrays(1)          # required for gl_VertexID fullscreen draw
    pts_vao, pts_vbo, pts_count = _setup_points(sim)

    # Density texture
    nz, ny, nx = int(sim.NZ), int(sim.NY), int(sim.NX)
    tex_density = _create_density_3d_texture(nx, ny, nz)
    _init_density(sim)
    ...
    # Match your point viewer’s world-space grid convention: [0..NX],[0..NY],[0..NZ]
    ...
    # Frame the volume with the orbit camera
    ...
    while not glfw.window_should_close(window):
        ...
        _upload_density_3d(sim.densityfield, tex_density)
        ...
        aspect = w / max(1, h)
        proj = pyrr.matrix44.create_perspective_projection(45.0, aspect, 0.1, 400.0, dtype=np.float32)
        ...
        if show_skybox:
            ...
            loc_sky_inv = gl.glGetUniformLocation(sky_prog, "uInvViewProj")
            gl.glUniformMatrix4fv(loc_sky_inv, 1, gl.GL_FALSE, inv_vp_sky)
            gl.glBindVertexArray(sky_vao)
            gl.glDrawArrays(gl.GL_TRIANGLES, 0, 3)
            gl.glBindVertexArray(0)

        # Volume haze (no depth, blended)
        gl.glDisable(gl.GL_DEPTH_TEST)
        gl.glUseProgram(vol_prog)
        gl.glActiveTexture(gl.GL_TEXTURE0)
        gl.glBindTexture(gl.GL_TEXTURE_3D, tex_density)
